-- 💜 BranzZ Hub Duels v1 💜
-- Purple Edition - Vertical Single Column UI
...
local BRANZZ_TEXT = "BranzZ Hub"
local function getBranzzProgress(percent)
    local total = #BRANZZ_TEXT
    local adjusted = math.min(percent * 1.5, 100)
    local chars = math.floor((adjusted / 100) * total)
    return string.sub(BRANZZ_TEXT, 1, chars)
end
...
local function executeSteal(prompt, name)
    ...
    progressConnection = RunService.Heartbeat:Connect(function()
        ...
        if ProgressPercentLabel then
            ...
            ProgressPercentLabel.Text = getBranzzProgress(pct)
        end
    end)
    task.spawn(function()
...
local function startAutoRightPath()
    if autoRightConn then autoRightConn:Disconnect() autoRightConn = nil end
    autoRightConn = RunService.RenderStepped:Connect(function()
        if not Enabled.AutoRight then return end
        local char = Player.Character
        if char and char:FindFirstChild("HumanoidRootPart") then
            char.HumanoidRootPart.CFrame = char.HumanoidRootPart.CFrame * CFrame.new(AUTO_STEP, 0, 0)
        end
    end)
end

local function stopAutoRightPath()
    if autoRightConn then autoRightConn:Disconnect() autoRightConn = nil end
end

local function startAutoLeftPath()
    if autoLeftConn then autoLeftConn:Disconnect() autoLeftConn = nil end
    autoLeftConn = RunService.RenderStepped:Connect(function()
        if not Enabled.AutoLeft then return end
        local char = Player.Character
        if char and char:FindFirstChild("HumanoidRootPart") then
            char.HumanoidRootPart.CFrame = char.HumanoidRootPart.CFrame * CFrame.new(-AUTO_STEP, 0, 0)
        end
...
local function stopAutoLeftPath()
    if autoLeftConn then autoLeftConn:Disconnect() autoLeftConn = nil end
end

-- Aliases para compatibilidade com o resto do código
local function startStealPath(dir)
    if dir == "right" then startAutoRightPath()
    else startAutoLeftPath() end
end
local function stopStealPath()
    -- Para os dois (usado quando um substitui o outro)
    stopAutoRightPath()
    stopAutoLeftPath()
end
...
-- ============================================================
if CoreGui:FindFirstChild("BranzZHub") then
    CoreGui.BranzZHub:Destroy()
end
...
local PINK       = Color3.fromRGB(138, 43, 226)   -- roxo vibrante (era laranja)
local PINK_LIGHT = Color3.fromRGB(180, 100, 255)  -- roxo claro
local PINK_DARK  = Color3.fromRGB(90, 20, 160)    -- roxo escuro
local BG_DARK    = Color3.fromRGB(10, 8, 18)      -- preto azulado
local BG_MID     = Color3.fromRGB(15, 10, 25)     -- cinza escuro
local SECTION_BG = Color3.fromRGB(8, 6, 16)       -- preto quase puro

-- Size constants for single-column vertical layout
local PANEL_W      = 230   -- single column width
local BANNER_H     = 28
local TOGGLE_H     = 28
local SLIDER_H     = 40
local SECTION_H    = 20
...
Create("Frame", {
    ...
    Create("UIGradient", {
        Color = ColorSequence.new{
            ...
            ColorSequenceKeypoint.new(0.4, Color3.fromRGB(22, 12, 42)),
            ColorSequenceKeypoint.new(0.8, Color3.fromRGB(28, 15, 50)),
...
for i = 1, 18 do
    ...
    local p = Create("Frame", {
        ...
        BackgroundTransparency = math.random(50,85)/100,
        ZIndex = 1,
        Parent = particleBg
...
chGrad.Color = ColorSequence.new({
    ...
    ColorSequenceKeypoint.new(1,    Color3.fromRGB(0, 220, 80)),
})

-- Animação da borda girando entre verde e roxo
local bzColorTime = 0
task.spawn(function()
    local r = 0
    while chCircle.Parent do
        r = (r + 2) % 360
        bzColorTime = bzColorTime + 0.03
        chGrad.Rotation = r
        -- Pulsa entre verde e roxo violeta
        local t = (math.sin(bzColorTime) + 1) / 2
        local g1 = Color3.new(0, 0.86, 0.31)       -- verde
        local g2 = Color3.fromRGB(138, 43, 226)    -- roxo violeta
        local mid1 = Color3.new(0.54, 0.17, 0.89)  -- roxo
        local mid2 = Color3.new(0, 0.86, 0.31)     -- verde
        chGrad.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0,   g1:Lerp(g2, t)),
            ColorSequenceKeypoint.new(0.5, mid1:Lerp(mid2, t)),
            ColorSequenceKeypoint.new(1,   g1:Lerp(g2, t)),
        })
        task.wait(0.02)
    end
end)
...
-- Animação do texto BZ pulsando entre verde e roxo
task.spawn(function()
    local t2 = 0
    while chText and chText.Parent do
        t2 = t2 + 0.04
        local lerpT = (math.sin(t2) + 1) / 2
        chText.TextColor3 = Color3.fromRGB(0, 220, 80):Lerp(Color3.fromRGB(180, 100, 255), lerpT)
        task.wait(0.03)
    end
end)
...
chBtn.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(chCircle, TweenInfo.new(0.2), {
        BackgroundColor3 = uiVisible and Color3.fromRGB(12, 8, 22) or Color3.fromRGB(30, 15, 55)
    }):Play()
end)
...
local DiscordBtn = Create("TextButton", {
    BackgroundColor3 = Color3.fromRGB(0, 90, 180), -- azul
    ...
    Create("UIStroke", {Color=Color3.fromRGB(0, 140, 255), Thickness=1.5}) -- borda azul clara
})

local discordCopied = false
DiscordBtn.MouseButton1Click:Connect(function()
    if discordCopied then return end
    pcall(function() setclipboard("https://discord.gg/69xw56bg2J") end)
    pcall(function() if syn and syn.clipboard then syn.clipboard.set("https://discord.gg/69xw56bg2J") end end)
    discordCopied = true
    DiscordBtn.Text = "Copiado!"
    DiscordBtn.TextColor3 = Color3.fromRGB(0, 255, 120) -- verde ao copiar
    task.spawn(function()
        task.wait(2)
...
local ScrollFrame = Create("ScrollingFrame", {
    ...
    ScrollBarImageColor3 = PINK_LIGHT,
    BorderSizePixel = 0,
    ZIndex = 5,
    Parent = PanelFrame
}, {
...
local function CreateToggle(parent, labelText, enabledKey, callback, order)
    local row = Create("Frame", {
        BackgroundColor3 = Color3.fromRGB(14, 8, 28),
    ...
    local toggleBg = Create("Frame", {
        ...
        BackgroundColor3 = isOn and PINK or Color3.fromRGB(30, 15, 55),
        ZIndex = 7,
        Parent = row
    }, {Create("UICorner", {CornerRadius=UDim.new(1,0)})})
    ...
    local function setVisual(state, skipCb)
        isOn = state
        TweenService:Create(toggleBg, TweenInfo.new(0.25), {BackgroundColor3 = isOn and PINK or Color3.fromRGB(30, 15, 55)}):Play()
...
local function CreateSlider(parent, labelText, minVal, maxVal, valueKey, callback, order)
    ...
    local labelFrame = Create("TextLabel", {
        ...
        TextColor3 = Color3.fromRGB(200, 170, 240),
...
-- ============================================================
-- BANNER WATERMARK NO TOPO DA TELA (clicável, copia discord)
-- ============================================================
local topBanner = Instance.new("Frame")
topBanner.Name = "TopWatermark"
topBanner.Size = UDim2.new(0, 520, 0, 28)
topBanner.Position = UDim2.new(0.5, -260, 0, 6)
topBanner.BackgroundColor3 = Color3.fromRGB(8, 5, 18)
topBanner.BorderSizePixel = 0
topBanner.ZIndex = 100
topBanner.Parent = ScreenGui
Instance.new("UICorner", topBanner).CornerRadius = UDim.new(0, 10)

-- Borda animada roxa/verde no banner
local topStroke = Instance.new("UIStroke", topBanner)
topStroke.Thickness = 2
topStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
local topGrad = Instance.new("UIGradient", topStroke)
...
-- Texto do banner
local topBannerLabel = Instance.new("TextButton")
topBannerLabel.BackgroundTransparency = 1
topBannerLabel.Size = UDim2.new(1, 0, 1, 0)
topBannerLabel.Font = Enum.Font.GothamBold
topBannerLabel.Text = "💜 BranzZ Duels V1  ||  esse script e gratis somente em  ||  https://discord.gg/69xw56bg2J"
topBannerLabel.TextColor3 = Color3.fromRGB(200, 150, 255)
topBannerLabel.TextSize = 11
topBannerLabel.ZIndex = 101
topBannerLabel.AutoButtonColor = false
topBannerLabel.Parent = topBanner

-- Animação de cor do texto
task.spawn(function()
    local t = 0
    while topBannerLabel and topBannerLabel.Parent do
        t = t + 0.04
        local lerp = (math.sin(t) + 1) / 2
        topBannerLabel.TextColor3 = Color3.fromRGB(180, 100, 255):Lerp(Color3.fromRGB(0, 220, 80), lerp)
        task.wait(0.03)
    end
end)

-- Copia discord ao clicar no banner
local bannerCopied = false
topBannerLabel.MouseButton1Click:Connect(function()
    if bannerCopied then return end
    pcall(function() setclipboard("https://discord.gg/69xw56bg2J") end)
    pcall(function() if syn and syn.clipboard then syn.clipboard.set("https://discord.gg/69xw56bg2J") end end)
    bannerCopied = true
    local oldText = topBannerLabel.Text
    topBannerLabel.Text = "✅  Link copiado! discord.gg/69xw56bg2J"
    topBannerLabel.TextColor3 = Color3.fromRGB(0, 220, 80)
    task.spawn(function()
        task.wait(2.5)
        topBannerLabel.Text = oldText
        bannerCopied = false
    end)
end)

-- ============================================================
-- PROGRESS BAR (bottom of screen) — glowing rotating border style
-- ============================================================
local pbar = Instance.new("Frame")
pbar.Size = UDim2.new(0, PBAR_W, 0, 44)
pbar.Position = UDim2.new(0.5, -PBAR_W/2, 1, -110)
pbar.BackgroundColor3 = Color3.fromRGB(8, 5, 18)
...
-- Rotating gradient border stroke - roxo/verde
...
pGrad.Color = ColorSequence.new({
    ...
    ColorSequenceKeypoint.new(0.25, Color3.fromRGB(0, 150, 50)),
    ColorSequenceKeypoint.new(0.5,  Color3.fromRGB(180, 100, 255)),
    ColorSequenceKeypoint.new(0.75, Color3.fromRGB(0, 220, 80)),
...
-- "READY" / steal name label — left, vertically centred
...
ProgressPercentLabel.TextColor3 = Color3.fromRGB(180, 100, 255)
...
statRadius.TextColor3 = Color3.fromRGB(180, 100, 255)
...
pTrack.BackgroundColor3 = Color3.fromRGB(20, 12, 40)
...
local function CreateFloatingButton(labelText, icon, defaultPos, enabledKey, onToggle)
    local btnFrame = Create("Frame", {
        ...
        Create("UIGradient", {
            Color = ColorSequence.new{
                ColorSequenceKeypoint.new(0, Color3.fromRGB(25, 12, 50)),
    ...
    local function updateVisual(state)
        ...
        TweenService:Create(btnFrame, TweenInfo.new(0.2), {
            BackgroundColor3 = state and Color3.fromRGB(35, 15, 70) or Color3.fromRGB(10, 6, 22)
        }):Play()
        ...
        TweenService:Create(textLabel, TweenInfo.new(0.2), {
            TextColor3 = state and PINK or Color3.fromRGB(210, 200, 230)
        }):Play()
    end
